...
void LlmConfigSection::load_providers() {
    ...
    if (result.is_ok()) {
        for (const auto& p : result.value()) {
            ...
            QString display = is_fincept ? "Guardian AI" : p.provider;
            if (p.is_active) {
                display += "  ✓";
...
void LlmConfigSection::populate_form(const QString& provider) {
        ...
        if (p.provider.toLower() == provider.toLower()) {
            ...
            if (is_fincept) {
                ...
                if (stored.is_ok() && !stored.value().isEmpty()) {
                    ...
                    api_key_edit_->setPlaceholderText("Linked to your Guardian account: " + masked);
                } else {
                    api_key_edit_->setPlaceholderText("Login to your Guardian account to enable");
                }
    ...
    if (is_fincept) {
        auto stored = SettingsRepository::instance().get("fincept_api_key");
        if (stored.is_ok() && !stored.value().isEmpty())
            api_key_edit_->setPlaceholderText("Linked to your Guardian account: " + stored.value().left(8) + "...");
        else
            api_key_edit_->setPlaceholderText("Login to your Guardian account to enable");
        model_combo_->setVisible(false);
        fetch_btn_->setVisible(false);
...
void LlmConfigSection::on_delete_provider() {
    ...
    if (provider.toLower() == "fincept") {
        show_status("Cannot remove built-in Guardian provider", true);
        return;
    }
...
void LlmConfigSection::on_test_connection() {
    ...
    if (provider == "fincept") {
        ...
        auto stored = SettingsRepository::instance().get("fincept_api_key");
        if (stored.is_ok() && !stored.value().isEmpty())
            show_status("Guardian connected — API key active", false);
        else
            show_status("Not connected — login to your Guardian account first", true);
        return;
    }
...
void LlmConfigSection::on_fetch_models() {
    ...
    if (provider == "fincept") {
        show_status("Guardian manages models automatically", false);
        return;
    }

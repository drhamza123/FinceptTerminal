...
QWidget* ProfileScreen::build_support() {
    ...
    add_c(tr("GENERAL SUPPORT"), "support@guardian.local", 0, 0);
    add_c(tr("COMMERCIAL"), "support@guardian.local", 0, 1);
    add_c(tr("SECURITY"), "support@guardian.local", 1, 0);
    add_c(tr("LEGAL"), "support@guardian.local", 1, 1);
    cvl2->addLayout(cg);
    vl->addWidget(cp);
...
void ProfileScreen::show_delete_account_dialog() {
    ...
    auto first = QMessageBox::warning(
        ...
        tr("This will permanently delete your Guardian account (%1) and all associated data.\n\n"
        ...
        auth::UserApi::instance().delete_user_account(email, password, [self](auth::ApiResponse r) {
            ...
            if (r.success) {
                ...
                QMessageBox::critical(self, tr("Delete Failed"),
                                      tr("Account deletion failed: %1\n\nPlease contact support@guardian.local").arg(r.error));
            }
        });
    });

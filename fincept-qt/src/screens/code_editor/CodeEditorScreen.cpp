...
QWidget* CodeEditorScreen::build_toolbar() {
    ...
    mt5_deploy_btn_->setStyleSheet(QString("QPushButton { background:#1a3a1a; color:#00D66F; border:1px solid #00D66F;"
                                       ...
                                       .arg(fonts::TINY));
    connect(mt5_deploy_btn_, &QPushButton::clicked, this, &CodeEditorScreen::on_deploy_to_mt5);
    hl->addWidget(mt5_deploy_btn_);

    mt5_status_label_ = new QLabel("", bar);
    mt5_status_label_->setStyleSheet(QString("color:%1; font-family:%2; font-size:9px; font-weight:600;"
                                             "padding:0 8px;")
                                         .arg(colors::TEXT_TERTIARY(), fonts::DATA_FAMILY));
    hl->addWidget(mt5_status_label_);

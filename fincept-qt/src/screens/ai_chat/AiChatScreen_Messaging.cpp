...
void AiChatScreen::update_stats() {
    ...
    if (llm.is_configured()) {
        ...
        // Display names
        const QString prov_display = is_fincept ? tr("Guardian AI") : provider_raw.toUpper();
        const QString model_raw = llm.active_model();
        // For fincept, don't expose internal model name
        const QString model_display = is_fincept ? tr("Guardian AI") : model_raw;
        ...
        model_lbl_->setText(is_fincept ? tr("Managed by Guardian") : model_short);
        model_lbl_->setToolTip(is_fincept ? tr("Guardian AI — managed AI service") : model_raw);
        ...
        if (is_fincept) {
            hdr_model_lbl_->setText(tr("Guardian AI"));
            hdr_model_lbl_->setToolTip(tr("Guardian managed AI service\n\nChange in Settings > LLM Configuration"));
        } else {

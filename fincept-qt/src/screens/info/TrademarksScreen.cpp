...
TrademarksScreen::TrademarksScreen(QWidget* parent) : QWidget(parent) {
    ...
    pvl->addWidget(heading("1", "GUARDIAN TRADEMARKS"));
    pvl->addWidget(bullet("Guardian (TM)"));
    pvl->addWidget(bullet("AI Stock Guardian (TM)"));
    pvl->addWidget(bullet("Guardian Corp (TM)"));
    pvl->addWidget(bullet("Guardian Logo and associated visual identities"));
    ...
    pvl->addWidget(bullet("Referring to Guardian products in editorial or descriptive contexts"));
    pvl->addWidget(bullet("Linking to official Guardian resources"));
    pvl->addWidget(bullet("Academic or research references"));

    pvl->addSpacing(4);
    pvl->addWidget(body("Prohibited Uses:"));
    pvl->addWidget(bullet("Using Guardian marks to imply endorsement or affiliation"));
    pvl->addWidget(bullet("Modifying or altering any Guardian trademark"));
    pvl->addWidget(bullet("Using Guardian marks in domain names or product names"));
    pvl->addWidget(bullet("Creating confusingly similar marks"));

    pvl->addWidget(heading("4", "COPYRIGHT NOTICE"));
    pvl->addWidget(body("Copyright 2024-2026 Guardian Corp. All rights reserved."));
    ...
    pvl->addWidget(body("To report trademark infringement, contact: support@guardian.local"));

    pvl->addWidget(heading("7", "LEGAL DEPARTMENT"));
    pvl->addWidget(body("Guardian Corp — Legal Department"));
    pvl->addWidget(body("Email: support@guardian.local"));

...
AlgoTradingScreen::AlgoTradingScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(poll_timer_, &QTimer::timeout, this, [this]() {
        ...
        if (active_tab_ == 4 && mt5_fleet_)
            mt5_fleet_->refreshFleet();
    });
...
void AlgoTradingScreen::build_ui() {
    ...
    mt5_fleet_ = new MT5FleetScreen(this);
    ...
    content_stack_->addWidget(mt5_fleet_);
...
QWidget* AlgoTradingScreen::build_top_bar() {
    ...
    // Tab buttons
    QStringList tabs   = {"BUILDER", "MY STRATEGIES", "SCANNER", "DASHBOARD", "MT5 FLEET"};
    QStringList colors = {"#FF6B35", "#00E5FF", "#FFC400", "#00D66F", "#FF4444"};

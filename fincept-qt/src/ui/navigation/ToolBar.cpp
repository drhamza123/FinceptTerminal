...
ToolBar::ToolBar(QWidget* parent) : QWidget(parent) {
    ...
    sep();
    // Brand display — shown in the toolbar before the subtitle.
    fincept_label_ = mk(QStringLiteral("AI STOCK "));
    hl->addWidget(fincept_label_);
    branding_label_ = mk(QStringLiteral("GUARDIAN"));
...
QMenu* ToolBar::build_navigate_menu() {
    ...
    nav(tools, tr("Community Marketplace"), "community_marketplace");
...
QMenu* ToolBar::build_help_menu() {
    ...
    m->setStyleSheet(popup_ss());
    m->addAction(tr("About Guardian"), this, [this]() { emit navigate_to("about"); });

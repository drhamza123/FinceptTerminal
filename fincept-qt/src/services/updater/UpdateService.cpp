...
void UpdateService::check_for_updates(bool silent) {
    ...
    req.setHeader(QNetworkRequest::UserAgentHeader,
                  QString("AIStockGuardian/%1 (%2)").arg(local_version, platform_key));
...
void UpdateService::on_manifest_reply_finished() {
    ...
    if (!is_newer(local_version, remote_version)) {
        ...
        if (!silent_) {
            QMessageBox::information(
                dialog_parent(), QStringLiteral("AI Stock Guardian"),
    ...
    QString prompt = QStringLiteral("A new version of AI Stock Guardian is available.\n\n"
                                    "Current version: %1\nLatest version:  %2\n\n")
...
void UpdateService::start_download(const QString& url, const QString& expected_sha256) {
    ...
    pending_local_path_ = QDir(dir).filePath(file_name.isEmpty()
                                                 ? QStringLiteral("AIStockGuardian-update.tmp")
                                                 : file_name);
    ...
    req.setAttribute(QNetworkRequest::RedirectPolicyAttribute, QNetworkRequest::NoLessSafeRedirectPolicy);
    req.setHeader(QNetworkRequest::UserAgentHeader,
                  QString("AIStockGuardian/%1").arg(QApplication::applicationVersion()));
    QNetworkReply* reply = net_.get(req);
...
void UpdateService::show_error(const QString& text) {
    QMessageBox::warning(dialog_parent(), QStringLiteral("AI Stock Guardian"), text);
}

...
LlmResponse LlmService::fincept_async_request(const QString& user_message,
    ...
    if (!submit.success) {
        resp.error = "Guardian async submit failed: " + submit.error;
    ...
    if (submit_doc.isNull()) {
        resp.error = "Guardian async: failed to parse submit response";
        return resp;
    }
    ...
    if (task_id.isEmpty()) {
        resp.error = "Guardian async: no task_id in submit response";
        return resp;
    }
    LOG_INFO(kLlmFinceptTag, "Guardian async task_id: " + task_id);
    ...
    for (int i = 0; i < MAX_POLLS; ++i) {
        ...
        if (!poll.success) {
            LOG_WARN(kLlmFinceptTag, "Guardian async poll failed: " + poll.error);
            continue;
        }
        ...
        LOG_INFO(kLlmFinceptTag, QString("Guardian async poll %1 status=%2").arg(i + 1).arg(status));
        ...
        if (status == "completed") {
            ...
            if (response.isEmpty()) {
                resp.error = "Guardian async completed but response is empty";
                LOG_WARN(kLlmFinceptTag, "Guardian async task completed with empty response field");
                return resp;
            }
            ...
            if (!resp.content.isEmpty()) {
                LOG_INFO(kLlmFinceptTag, "Guardian: checking response for text-based tool calls");
                ...
                if (tool_result.has_value()) {
                    LOG_INFO(kLlmFinceptTag, "Guardian: text tool calls detected and executed");
        ...
        if (status == "failed") {
            ...
            resp.error = "Guardian async task failed: " + (err.isEmpty() ? "unknown error" : err);
            return resp;
        }
    }

    resp.error = "Guardian async timed out waiting for response";
    return resp;
}

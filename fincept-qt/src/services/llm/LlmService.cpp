...
void LlmService::ensure_config() const {
    ...
    if (system_prompt_.trimmed().isEmpty()) {
        system_prompt_ =
            "You are Guardian AI, the intelligent assistant embedded inside the AI Stock Guardian — "
            ...
            "    Report'), author (e.g. 'Guardian Research'), company, and date. Don't leave 'Analyst' "
...
QMap<QString, QString> LlmService::get_headers() const {
    ...
    if (p == "anthropic") {
        ...
        h["User-Agent"] = "AIStockGuardian/1.0"; // Cloudflare requires it.
    } else {
        if (!api_key_.isEmpty())
...
LlmResponse LlmService::do_request(const QString& user_message, const std::vector<ConversationMessage>& history) {
    ...
    if (provider_ == "anthropic") {
        ...
        if (resp.content.isEmpty()) {
            resp.error = "Guardian AI returned an empty response. Please try again.";
            LOG_WARN(kLlmSvcTag, "Guardian /research/chat returned empty choices or content");
            return resp;
        }

...
WindowFrame::WindowFrame(int window_id, QWidget* parent, const WindowId& adopted_uuid)
    ...
    setWindowTitle(profile == "default" ? QStringLiteral("AI Stock Guardian")
                                        : QStringLiteral("AI Stock Guardian [%1]").arg(profile));
    ...
    // ── MCP Hot-Reload WebSocket ─────────────────────────────────────────────
    auto* mcp_ws = new QWebSocket(QString(), QWebSocketProtocol::VersionLatest, this);
    connect(mcp_ws, &QWebSocket::textMessageReceived, this, [this](const QString& msg) {
        QJsonDocument doc = QJsonDocument::fromJson(msg.toUtf8());
        if (doc.isObject()) {
            auto obj = doc.object();
            if (obj["type"].toString() == "tool_installed") {
                LOG_INFO("Marketplace", "Tool installed: " + obj["name"].toString());
                // Could emit a signal here for McpServersScreen to reload
            }
        }
    });
    ...
    // The engine is created per-chart in MT5FleetChartPanel. The global
    // background thread for ZMQ push starts on first use. No global init needed.
    ...
    connect(toolbar, &ui::ToolBar::action_triggered, this, [this](const QString& action) {
        ...
        if (action == "new_window") {
            ...
            QString path = QFileDialog::getOpenFileName(
                ...
                tr("Guardian Layout (*.flayout *.fwsp);;All Files (*)"));
            if (!path.isEmpty()) {
...
void WindowFrame::update_window_title() {
    ...
    QString title = QStringLiteral("AI Stock Guardian");

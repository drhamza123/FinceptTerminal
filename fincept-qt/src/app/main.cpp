...
#    define FINCEPT_VERSION_STRING "0.0.0-dev"
    ...
    const QString profile_key = QString("AIStockGuardian-%1").arg(fincept::ProfileManager::instance().active());
    ...
    // Clean legacy v3 DB location (these paths are no longer live DBs)
    {
        ...
        const QString legacy1 = local_dir.section('/', 0, -3) + "/AIStockGuardian/fincept_settings.db";
        const QString legacy2 =
            QString(local_dir).replace("Guardian/AIStockGuardian", "AIStockGuardian") + "/fincept_settings.db";
    ...
    LOG_INFO("App", "AI Stock Guardian v4.0.3 starting...");
    ...
    if (db_result.is_err()) {
        ...
        // — eliminates flash/wrong-font-on-startup. Theme is always Obsidian.
        {
            ...
            tm.apply_theme("Guardian");
            LOG_INFO("App", "Theme: Guardian, font: " + family + " " + size_s);
        }
    ...
    // One-time migration: copy settings from old DB (Local\AIStockGuardian\fincept_settings.db)
    // to new DB (Roaming\Guardian\AIStockGuardian\fincept.db) if the new DB has no settings yet.
    {
        ...
        if (new_db_empty) {
            ...
            // AppLocalDataLocation = .../Local/Guardian/AIStockGuardian — strip to .../Local/AIStockGuardian
            QString old_db_path = local_base.section('/', 0, -3) + "/AIStockGuardian/fincept_settings.db";
            if (!QFile::exists(old_db_path)) {
                // Try without the org subfolder
                old_db_path = local_base.replace("Guardian/AIStockGuardian", "AIStockGuardian") + "/fincept_settings.db";
            }
            if (QFile::exists(old_db_path)) {
    ...
    if (setup_status.needs_setup) {
        ...
        setup_screen->setWindowTitle("AI Stock Guardian — First-Time Setup");

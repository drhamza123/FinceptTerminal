...
//   Windows : %LOCALAPPDATA%\com.guardian.aistockguardian\
...
//             %TEMP%\fincept_*
//   macOS   : ~/Library/Application Support/com.guardian.aistockguardian/
//             ~/Library/Preferences/com.fincept.FinceptTerminal.plist
//             ~/Library/Preferences/Fincept.plist (if present)
//             Keychain: com.guardian.aistockguardian service entries
//             $TMPDIR/fincept_*, /tmp/fincept_*
//   Linux   : ~/.local/share/com.guardian.aistockguardian/
//             ~/.config/Fincept/
//             /tmp/fincept_*
...
function cleanUserDataWindows()
{
    ...
    // 1. Main data root
    removeDirWindows(localAppData + "/com.guardian.aistockguardian");
...
function cleanUserDataMac()
{
    ...
    // 1. Main data root
    removeDirPosix(home + "/Library/Application Support/com.guardian.aistockguardian");
    ...
    removeDirPosix(home + "/Library/Caches/com.guardian.aistockguardian");
    removeDirPosix(home + "/Library/Caches/Fincept");

    // 4. Saved application state
    removeDirPosix(home + "/Library/Saved Application State/com.guardian.aistockguardian.savedState");

    // 5. Keychain: delete all entries under service "com.guardian.aistockguardian".
    //    security(1) removes one entry per call — loop until it fails (no more).
    runAndLog("/bin/bash", ["-c",
        "while /usr/bin/security delete-generic-password -s 'com.guardian.aistockguardian' >/dev/null 2>&1; do :; done; exit 0"
    ]);
...
function cleanUserDataLinux()
{
    ...
    removeDirPosix(xdgDat + "/com.guardian.aistockguardian");
    removeDirPosix(home   + "/.local/share/com.guardian.aistockguardian");
    ...
    removeDirPosix(xdgCch + "/com.guardian.aistockguardian");
